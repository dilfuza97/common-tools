...
resource "kubernetes_deployment" "jenkins_dilfuz_deployment" {
  depends_on = [
  ...
  metadata {
    name = "jenkins-dilfuz-deployment"
...
resource "kubernetes_service" "jenkins_dilfuz_service" {
  depends_on = ["kubernetes_namespace.service_tools"]

  metadata {
    name      = "jenkins-dilfuz-service"
    namespace = "${var.namespace}"
  }
